...
These Linux packages are unsigned x86_64 verification candidates. They inherit the Ubuntu runner and glibc baseline and therefore do not prove runtime compatibility with every Linux distribution. ARM packages, Flatpak, and Snap are not part of this workflow. Product repositories add protected signing and publication jobs only after this gate.
...
Every external Action is pinned to a full 40-character commit SHA and carries its human-readable immutable release version as a comment. `.github/dependabot.yml` opens grouped update pull requests for GitHub Actions, npm, both Python requirement roots, both Cargo roots, Dockerfiles, and Compose inputs. `.github/CODEOWNERS` routes workflow, policy, deployment, native, and tooling changes to the repository owner.

Repository settings are not encoded by workflow YAML. Maintainers must enable the Dependency Graph, Dependabot alerts and security updates, Code Scanning, Private Vulnerability Reporting, Secret Scanning with Push Protection, and a branch ruleset that requires the applicable Core, Profile, PostgreSQL, Desktop, and Security checks. The ruleset should require CODEOWNERS review for protected paths and disallow force pushes and tag movement. Record those settings in release evidence instead of claiming them from committed files alone.
...
Every workflow defaults to `contents: read`. CodeQL alone receives `security-events: write`, and the Release Validation job that creates attestations alone receives `attestations: write` plus `id-token: write`. Pull requests from forks do not require repository secrets. PostgreSQL uses the temporary user, password, and database declared in workflow YAML; those values exist only inside an isolated CI run and are not production credentials.

The workflows do not use cloud credentials, signing material, deployment tokens, package publication, or public release creation. Commands must not print database URLs or other secret values. Normal CI jobs run `git diff --exit-code` to detect unintended changes to versioned files.
